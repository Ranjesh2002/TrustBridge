...
def get_segment_weights(segment: str) -> Dict[str, float]:
    weights = {
        ...
        "cash_merchant":  {"social": 0.35, "psychometric": 0.30, "behavioral": 0.35},
        "new_merchant":   {"social": 0.45, "psychometric": 0.40, "behavioral": 0.15}
    }
    return weights.get(segment, weights["new_merchant"])


def compute_confidence(data_points: int, source_count: int, fraud_flag: bool, psychometric_complete: bool) -> float:
    ...
    confidence = (data_confidence * 0.5 + source_confidence * 0.4 + psychometric_bonus) * (1 - fraud_penalty)
...
def fuse_scores(merchant: Dict, social_result: Dict, psychometric_result: Dict, behavioral_result: Dict) -> Dict:
    ...
    if behavioral_result.get("sub_scores", {}).get("obligation_fulfillment", 0) > 0: sources += 1
    if merchant.get("esewa_registered") or merchant.get("khalti_registered"): sources += 1
    ...
    return {
        ...
        "sub_scores": {"social": social_score, "psychometric": psychometric_score, "behavioral": behavioral_score},
