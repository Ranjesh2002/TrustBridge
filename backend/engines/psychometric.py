...
def score_responses_deterministic(responses: Dict[str, str]) -> Dict:
    ...
    for trait in trait_scores:
        if trait_counts[trait] > 0:
            trait_scores[trait] = round(trait_scores[trait] / trait_counts[trait])

    return trait_scores


def analyze_with_gemini(merchant_name: str, responses: Dict[str, str], trait_scores: Dict[str, int]) -> Dict:
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        return _fallback_result(trait_scores, "No GEMINI_API_KEY set")
...
{{
  ...
  "strengths": "<key strength for lending decisions>"
        ...
        required = ["conscientiousness", "risk_aversion", "social_trust", "resilience", "planning", "psychometric_score"]
        ...
        return _fallback_result(trait_scores, str(e))


def _fallback_result(trait_scores: Dict, error: str) -> Dict:
    weighted = round(
...
def run_psychometric_assessment(merchant_id: str, merchant_name: str, responses: Dict[str, str]) -> Dict:
